...
A CSRF vulnerability allows an attacker to unknowingly perform state changes on a web application where the victim is authenticated, CSRF does not allow data theft as an attacker cannot view the response from the webpage. CSRF is different from XSS is in such a way that we are not actually stealing any data but only performing actions on behalf of the victim without him knowing. It occurs when an attacker can use an HTTP request to access a user’s information from another website and use that information to act on the user’s behalf. This typically relies on the victim being previously authenticated on the target website where the action is submitted.

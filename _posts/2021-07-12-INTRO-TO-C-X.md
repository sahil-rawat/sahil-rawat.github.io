...
This is the tenth post in the series [Intro To C](https://www.sahilsinghrawat.com/posts/INTRO-TO-C-I/) In the last post we learned Structs

In this article, we will learn about Files

## File

We've already seen a couple of examples of I/O with scanf() and printf() for doing I/O at the console (screen/keyboard).
...
We've been using these implicitly already, it turns out. For example, these two calls are the same:
...
Also, you'll notice that both stdout and stderr go to the screen. While this seems at first either like an oversight or redundancy, it isn't. Typical operating systems allow you to *redirect* the output of either of those into different files, and it can be convenient to be able to separate error messages from the regular non-error output.
...
For this reason, you should send serious error messages to stderr instead of stdout. More on how to do that later.

### Reading Text Files

Streams are largely categorized in two different ways: *text* and *binary*.

Text streams are allowed to do a significant translation of the data, most notable translations of newlines to their different representations. Text files are logically a sequence of *lines* separated by newlines. To be portable, your input data should always end with a newline.

But the general rule is that if you're able to edit the file in a regular text editor, it's a text file. Otherwise, it's binary.
...
(Line 10 can be understood by breaking it down starting with the innermost-nested parens. The first thing we do is assign the result of fgets() into c, and *then* we compare that against EOF. We've just crammed it into a single line.)
...
We can get an entire line at once by using fgets() For arguments, it takes a pointer to a char buffer to hold bytes, a maximum number of bytes to read, and a FILE* to read from. It returns NULL on the end-of-file or error.

Let's do a similar loop as before, except let's have a multiline file and read it in a line at a time. Here's a file quote.txt:
...
You know how you can get formatted output with printf()
...
Hello, world!
```
Since stdout is a file, you could replace line 8 with:
`fp = stdout;`

and the program would have outputted to the console instead of to a file.

### Binary File I/O

So far we've just been talking text files. But there's another type of files we mentioned early on called *binary* files or binary streams.
...
To demo, we'll write a couple of programs. One will write a sequence of byte values to disk all at once. And the second program will read a byte at a time and print them out.
...
Running the program gives us a file output.bin, but opening it in a text editor doesn't show anything friendly! It's binary data, not text.
...
It's not portable to just fwrite() an entire struct out to a file when you don't know where the padding will end up.
...
This has something to do with what's called the *endianness* of the architecture. Some write the most significant bytes first, and some the least significant bytes first.

This means that if you write a multibyte number out straight from memory, you can't do it in a portable way.

A similar problem exists with the floating point. Most systems use the same format for their floating point numbers, but some do not. No guarantees!
